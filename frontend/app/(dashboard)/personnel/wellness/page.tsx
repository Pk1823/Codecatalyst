  ...
  if (submittedResult) {
    return (
      <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in zoom-in-95 duration-200">
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6 sm:p-8 text-center space-y-6">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950 p-5 text-left space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <div className="text-right">
                <span className="text-2xl font-mono font-bold text-white">
                  {submittedResult.score} <span className="text-xs text-slate-500 font-normal">/ 100</span>
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
              <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                <span className="text-slate-400 block text-[10px]">Stress Indicator</span>
                <span className="font-semibold text-white">{submittedResult.stressLevel}</span>
              </div>
              <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                <span className="text-slate-400 block text-[10px]">Fatigue Level</span>
                <span className="font-semibold text-white">{submittedResult.fatigueLevel}</span>
              </div>
              <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                <span className="text-slate-400 block text-[10px]">Workload</span>
                <span className="font-semibold text-white">{submittedResult.workloadStatus}</span>
              </div>
              <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                <span className="text-slate-400 block text-[10px]">Recovery</span>
                <span className="font-semibold text-white">{submittedResult.recoveryStatus}</span>
              </div>
            </div>
            ...
            <div className="p-3 rounded-lg bg-slate-900 text-xs text-slate-300 border border-slate-800">
              ...
              <p className="text-slate-400">{submittedResult.recommendation}</p>
            </div>
          </div>
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/personnel"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 text-xs font-semibold transition-colors"
            >
            ...
            <Link
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-800 hover:bg-slate-750 text-slate-200 px-4 py-2 text-xs font-semibold transition-colors"
            >
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 flex items-center gap-3">
        ...
        <div className="text-xs text-slate-300">
          <span className="font-semibold text-white">Participation is voluntary.</span> Your inputs help
        ...
        <div className="space-y-6">
            ...
            {currentStep.options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`w-full p-3.5 rounded-lg border text-left transition-colors flex items-start justify-between gap-3 ${
                    isSelected
          ...
          {currentStepIndex === WELLNESS_STEPS.length - 1 && (
            <div className="pt-2">
              ...
              <textarea
                ...
                className="w-full rounded-lg border border-slate-700 bg-slate-950 p-2.5 text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-emerald-500"
              />
            </div>
          )}
          ...
          <div className="pt-4 border-t border-slate-800 flex items-center justify-between gap-3">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-400 hover:bg-slate-800 disabled:opacity-30 transition-colors"
            >
            ...
            {currentStepIndex < WELLNESS_STEPS.length - 1 ? (
              <button
                type="button"
                onClick={nextStep}
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-sm transition-colors"
              >
                <span>Continue</span>
                <ArrowRight className="h-3.5 w-3.5" />
              </button>
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-sm transition-colors disabled:opacity-40"
              >
      ...
      <div className="text-center text-[11px] text-slate-500 max-w-md mx-auto">
