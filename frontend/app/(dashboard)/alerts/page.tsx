    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-700 hover:bg-slate-800 text-xs font-medium text-slate-300 transition-colors self-start sm:self-auto"
        >
          ...
          <div
            ...
            className={`p-4 rounded-xl border transition-colors flex items-start justify-between gap-4 ${
              item.isRead
                ...
                : "border-slate-700 bg-slate-900/80 ring-1 ring-emerald-500/20"
            }`}
          >
            <div className="space-y-1.5 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-[10px] rounded bg-slate-800 text-slate-300 px-1.5 py-0.5 font-mono">
                  {item.category}
                </span>
              ...
              {item.contributingIndicators && (
                <div className="flex flex-wrap gap-1 pt-1">
                  {item.contributingIndicators.map((ci, i) => (
                    <span
                      key={i}
                      className="text-[10px] rounded bg-slate-800 px-2 py-0.5 text-slate-300 font-mono"
                    >
                      {ci}
                    </span>
                  ))}
            ...
            <div className="flex items-center gap-2 shrink-0">
              {item.personnelId && (
                <Link
                  ...
                  className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-medium hover:bg-emerald-500 flex items-center gap-1 transition-colors"
                >
                  <span>Review</span>
