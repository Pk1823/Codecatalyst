...
export async function POST(req: NextRequest) {
  try {
    ...
    // Support both direct questionnaire options from mobile and manual category ratings
    let consecDays = 20;
    ...
    const recommendations = (prediction.recommendations || []).map((r) => r.description || r.title);
