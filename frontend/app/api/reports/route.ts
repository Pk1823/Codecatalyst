...
export async function GET(req: NextRequest) {
  try {
    ...
    // Personnel cannot access system/unit wide reports
    if (role === "PERSONNEL") {
      throw new AuthError("Forbidden: Personnel role cannot access aggregated operational reports", 403);
    }

    const searchParams = req.nextUrl.searchParams;
    const unitId = searchParams.get("unitId") || undefined;

    // 1. Unit Level Summaries
    ...
    // 4. Welfare Case Lifecycle Stats
    ...
    // 7. Wellness Assessment Monthly Trends (past records)
    ...
    for (const a of assessments) {
      const monthStr = new Date(a.createdAt).toISOString().slice(0, 7); // YYYY-MM
