  ...
  const handleLiveGoogleRedirect = async () => {
    ...
    try {
      ...
      if (data.url) {
        ...
        throw new Error("Could not retrieve Google OAuth endpoint.");
      }
    } catch (err: any) {
        ...
        <div className="p-6">
          <div className="text-center mb-6">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              to continue to <span className="font-medium text-emerald-600 dark:text-emerald-400">MissionWell AI (India CAPF)</span>
            </p>
          </div>
            ...
            <div className="space-y-2 mb-5">
              {PRECONFIGURED_GOOGLE_ACCOUNTS.map((acc) => (
                <button
                  ...
                  className="w-full text-left p-3.5 rounded-2xl border border-slate-200/80 dark:border-navy-700/60 hover:border-blue-500/50 dark:hover:border-blue-500/50 hover:bg-blue-50/40 dark:hover:bg-navy-800/60 transition-all flex items-center justify-between group"
                >
          ...
          <div className="pt-3 border-t border-slate-100 dark:border-navy-800 text-[11px] text-slate-500 dark:text-slate-400 space-y-1.5">
