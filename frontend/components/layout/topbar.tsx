    ...
    <>
      ...
      <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200/80 dark:border-slate-800/80 bg-white/90 dark:bg-[#090D16]/90 px-4 sm:px-6 backdrop-blur-xl transition-colors shadow-xs">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          {/* Google SSO Verified Badge (Clickable for full dataset) */}
          {googleSsoData && (
            <button
              onClick={() => setDatasetModalOpen(true)}
              className="hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-50/90 dark:bg-blue-950/60 border border-blue-200/80 dark:border-blue-800/60 text-[10px] font-mono shadow-2xs hover:scale-[1.03] active:scale-[0.98] transition-all cursor-pointer"
              title={`Click to view Google Account & Full Dataset: ${googleSsoData.email} (${googleSsoData.role.replace("_", " ")})`}
            >
